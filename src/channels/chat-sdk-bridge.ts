...
export interface ChatSdkBridgeConfig {
  ...
  /**
   * Namespace prefix for this adapter's Chat SDK state (subscriptions, locks,
   * queues, KV). Required when more than one bot on the same platform shares
   * identical thread ids — e.g. a second Telegram bot, where Telegram's
   * chat_id is the same regardless of which bot the user talks to, so without
   * a prefix the two bots' subscription/lock rows would collide. Defaults to
   * '' (single-instance channels are unaffected).
   */
  statePrefix?: string;
}
...
export function createChatSdkBridge(config: ChatSdkBridgeConfig): ChannelAdapter {
  ...
  const bridge: ChannelAdapter = {
    ...
    async setup(hostConfig: ChannelSetup) {
      ...
      state = new SqliteStateAdapter(config.statePrefix);
