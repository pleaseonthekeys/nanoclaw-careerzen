...
export class SqliteStateAdapter implements StateAdapter {
  ...
  /**
   * Optional namespace prepended to every thread_id / key written to the
   * shared chat_sdk_* tables. Channels that can run more than one bot instance
   * on the same platform (e.g. a second Telegram bot) pass a distinct prefix so
   * their subscriptions, locks, queues, and KV don't collide on identical
   * thread ids (Telegram's chat_id is the same regardless of which bot the user
   * talks to). Defaults to '' — single-instance channels are unaffected.
   */
  constructor(private keyPrefix = '') {}

  /** Namespace a thread_id / key with the per-instance prefix. */
  ...
  async acquireLock(threadId: string, ttlMs: number): Promise<Lock | null> {
    const now = Date.now();
    const tid = this.k(threadId);
    const token = crypto.randomUUID();
    const expiresAt = now + ttlMs;
    this.db.prepare('DELETE FROM chat_sdk_locks WHERE thread_id = ? AND expires_at < ?').run(tid, now);
    const result = this.db
      .prepare('INSERT OR IGNORE INTO chat_sdk_locks (thread_id, token, expires_at) VALUES (?, ?, ?)')
      .run(tid, token, expiresAt);
    if (result.changes === 0) return null;
    // Return the caller-facing (unprefixed) threadId; release/extend re-apply
    // the prefix so the Lock stays portable.
