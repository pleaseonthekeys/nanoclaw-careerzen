...
async function spawnContainer(session: Session): Promise<void> {
  ...
  // Scaffold the per-group filesystem + container_configs row BEFORE
  // materializeContainerJson reads it. Groups created outside the setup flow
  // (e.g. a bare `ncl groups create`) have no config row yet, and materialize
  // throws on a missing row — which used to happen before buildMounts' own
  // (idempotent) initGroupFilesystem call could create it, leaving the group
  // permanently unspawnable. Calling it here makes spawn self-healing; the
  // later call in buildMounts is then a no-op.
