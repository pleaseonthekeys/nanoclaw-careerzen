...
export interface ResourceDef {
  ...
  idColumn: string;
  /**
   * Optional generator for the auto-assigned primary key. Defaults to
   * randomUUID(). Override when the id has downstream format constraints —
   * e.g. agent_groups, whose id is used verbatim as the OneCLI agent
   * identifier, which must start with a letter (a UUID can start with a
   * digit and gets rejected with a 400).
   */
  idGenerator?: () => string;
  /**
    ...
    for (const col of def.columns) {
      if (col.generated) {
        if (col.name === def.idColumn) {
          values[col.name] = def.idGenerator ? def.idGenerator() : randomUUID();
