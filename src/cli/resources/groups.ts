...
registerResource({
  ...
  idColumn: 'id',
  scopeField: 'id',
  // The agent group id is used verbatim as the OneCLI agent identifier, which
  // must start with a letter (1-50 chars, lowercase/digits/hyphens). A bare
  // UUID can start with a digit and gets rejected with a 400 at container
  // spawn (ensureAgent). Match the `ag-...` format the in-app creation paths
  // already use (see createNewAgentGroup).
  idGenerator: () => `ag-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
  columns: [
    { name: 'id', type: 'string', description: 'Auto: ag-<ts>-<rand>.', generated: true },
    {
      name: 'name',
      type: 'string',
